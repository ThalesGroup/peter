import { EvaluationVersionApi } from '../../../../../src/module/evaluations/core/domain/ports/EvaluationVersionApi';
import {
  EvaluationType,
  EvaluationVersion,
...
import { LoadEvaluationVersionList } from '../../../../../src/module/shared/core/domain/dependencies';

export const STANDARD_EVALUATION_VERSION: EvaluationVersion = {
  id: '1',
  version: '1.0.0',
  types: [
    {
...
export class FakeEvaluationVersionApi implements EvaluationVersionApi {
  private readonly evaluationVersions: EvaluationVersion[] = [];

  async load(): Promise<EvaluationVersion[]> {
    return this.evaluationVersions;
  }
  ...
  initWith(
    ...
    evaluationVersionList.forEach((version) => {
      this.evaluationVersions.push(version);
    });
  }
}

export const createFakeLoadEvaluationVersionList =
  (
    willReturnEvaluationVersionList: EvaluationVersionList,
    { delay = 0 }: { delay?: number } = {}
  ): LoadEvaluationVersionList =>
  () => {
    return new Promise((resolve) => {
      if (delay === 0) {
        resolve(willReturnEvaluationVersionList);
        return;
      }
      setTimeout(() => resolve(willReturnEvaluationVersionList), delay);
    });
  };
